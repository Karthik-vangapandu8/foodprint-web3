...
const blockchainExplorerURL = process.env.ALGORAND_EXPLORER_URL;
...
var recoveredAccount1, recoveredAccount2, account1, account2;
...
if (account1_mnemonic && account2_mnemonic) {
  ...
  try {
    ...
    console.log('Error recovering Algorand accounts:', error.message);
    console.log('Blockchain features will be disabled.');
  }
} else {
  console.log('Algorand account mnemonics not configured - blockchain features will be disabled.');
}
...
if (environment == 'TESTNET') {
  token = {
    'X-API-Key': process.env.TESTNET_ALGOD_API_KEY, // https://nodely.io/docs/free/start - free service does not require tokens
  };
  ...
  //if DEV
  token = process.env.DEV_ALGOD_API_KEY; // https://nodely.io/docs/free/start - free service does not require tokens
...
router.post('/app/harvest/save/blockchain', async function (req, res) {
  // Check if Algorand accounts are configured
  if (!recoveredAccount1 || !recoveredAccount2) {
    // Algorand not configured - return error message directing user to use wallet
    return res.status(400).json({
      success: false,
      message: 'Algorand blockchain not configured. Please use wallet-based signing instead.',
...
function updateHarvestBlockchainHash(logId, supplyChainData, user, transactionId) {
  ...
  let data = {
    ...
    blockchain_explorer_url: blockchainExplorerURL + '/' + transactionId,
  };
  try {
...
// ==================== NEW WALLET-BASED BLOCKCHAIN ROUTES ====================

const { ethers } = require('ethers');

/**
 * Record harvest to blockchain using user's connected wallet
 * POST /app/harvest/save/blockchain/wallet
 */
router.post('/app/harvest/save/blockchain/wallet', async function (req, res) {
  try {
    if (!req.user) {
    ...
    const { logID, signature, message, walletAddress, blockchainData } = req.body;
    ...
    // Get user record (optional - wallet address from request is what matters)
    ...
    // Use wallet address from request (the one that actually signed)
    ...
    // Create blockchain hash of data
    const supplyChainData = JSON.stringify(blockchainData || req.body);
    const supplyChainDataHash = crypto
      .createHash('sha256')
    ...
    // Update harvest entry with wallet signature
    const data = {
      harvest_logid: logID,
      harvest_BlockchainHashID: supplyChainDataHash,
      harvest_BlockchainHashData: supplyChainData,
      harvest_added_to_blockchain_date: logdatetime,
      harvest_bool_added_to_blockchain: true,
      harvest_added_to_blockchain_by: req.user.email,
      harvest_blockchain_uuid: signature, // Use signature as transaction ID
      blockchain_explorer_url: `wallet://${userWalletAddress}`,
    };

    await models.FoodprintHarvest.update(data, {
      where: {
        harvest_logid: logID,
      },
    });
    ...
    console.log(
      `Harvest ${logID} recorded to blockchain with wallet ${userWalletAddress}`
    );

    res.status(201).json({
      success: true,
      message: 'Harvest entry signed and recorded with your wallet!',
      harvestLogid: logID,
      walletAddress: userWalletAddress,
      signature: signature,
    });
  } catch (error) {
    console.error('Wallet blockchain error:', error);
    res.status(500).json({
      success: false,
      message: 'Error recording to blockchain: ' + error.message,
    });
  }
});

/**
 * Record storage to blockchain using user's connected wallet
 * POST /app/storage/save/blockchain/wallet
 */
router.post('/app/storage/save/blockchain/wallet', async function (req, res) {
  try {
    if (!req.user) {
      return res.status(401).json({
        success: false,
    ...
    // Update storage entry with wallet signature
    const data = {
      storage_logid: logID,
      storage_BlockchainHashID: supplyChainDataHash,
      storage_BlockchainHashData: supplyChainData,
      storage_added_to_blockchain_date: logdatetime,
      storage_bool_added_to_blockchain: true,
      storage_added_to_blockchain_by: req.user.email,
      storage_blockchain_uuid: signature, // Use signature as transaction ID
      blockchain_explorer_url: `wallet://${userWalletAddress}`,
    };

    await models.FoodprintStorage.update(data, {
      where: {
        storage_logid: logID,
      },
    });
    ...
    console.log(
      `Storage ${logID} recorded to blockchain with wallet ${userWalletAddress}`
    );

    res.status(201).json({
      success: true,
      message: 'Storage entry signed and recorded with your wallet!',
      storageLogid: logID,
      walletAddress: userWalletAddress,
      signature: signature,
    });
