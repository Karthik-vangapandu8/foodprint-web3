...
const tw_client = (accountSid && authToken) ? require('twilio')(accountSid, authToken) : null;
...
/* GET voice-only harvest page (for illiterate farmers) */
router.get(
  '/voice',
  ...
  function (req, res, next) {
    if (
      ...
      res.render('harvest-voice', {
        page_title: 'FoodPrint - Voice Harvest Entry',
        user: req.user,
        page_name: 'harvest-voice',
      });
    } else {
...
router.post(
  ...
  function (req, res) {
    ...
    if (!result.isEmpty()) {
      ...
      try {
        // Check if DigitalOcean is configured for file uploads
        if (!isDigitalOceanConfigured) {
          // Skip file upload for local testing
          console.log('⚠️  Skipping file upload - DigitalOcean not configured');
          harvest_entry['harvest_image_url'] = '/img/placeholder.jpg'; // Placeholder for local testing
          
          models.FoodprintHarvest.create(harvest_entry)
            .then(_ => {
              req.flash(
                'success',
                'New Harvest entry added successfully! Harvest ID = ' + harvest_logid_uuid + ' (Image upload skipped - local testing mode)'
              );
              res.redirect('/app/harvest');
            })
            .catch(err => {
          ...
          // DigitalOcean is configured - proceed with file upload
  ...
  if (req.body.harvestURL) {
    try {
      ...
      if (tw_client) {
        ...
        console.log('Twilio not configured - skipping media fetch');
        createHarvestEntry(harvest_entry);
      }
  ...
  async function(req, res, next) {
    try {
      ...
      if (!url || url.includes('placeholder') || !url.startsWith('http')) {
        // Return a default hash for placeholder images
        const hash256 = crypto.createHash('sha256').update('placeholder').digest('base64');
        return res.send(hash256);
      }
